  ...
  <thead>
    <tr>
      <th
        ...
        v-if="massActions.length > 0 && dataCollection.length > 0"
      >
        <span class="checkbox">
          ...
          <template v-else>
            <input type="checkbox" :key="Math.random()" @change="selectAllRows(true)" />

            <label
              :class="`checkbox-${selectedTableRows.length > 0 ? 'dash' : 'view'}`"
              for="checkbox"
            ></label>
          </template>
        </span>
      ...
      <template v-for="(column, index) in columns">
        <th
          ...
          :class="[
            column.class ? column.class : column.index,
