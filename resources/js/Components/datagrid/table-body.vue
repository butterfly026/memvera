    ...
    <template v-if="resultLoaded">
      ...
      >
        <td v-if="massActions.length > 0">
          <span class="checkbox">
            <template
              v-if="selectedTableRows.filter((item, index) => item == row.id).length"
            >
              <input
                type="checkbox"
                checked="checked"
...
export default {
  ...
  methods: {
    ...mapActions('data', ["selectTableRow"]),
    ...
    performAjax: function ({ event, route, method, type }) {
      this.$http[method.toLowerCase()](route)
        .then((response) => {
          ...
          if (type == "download") {
            this.download(response.data.fileName, response.data.fileContent);
          } else {
    ...
    download: function (fileName, fileContent) {
      ...
      dlAnchorElem.setAttribute("href", `data:text/plain;charset=utf-8,${fileContent}`);
