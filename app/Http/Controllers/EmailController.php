...
class EmailController extends Controller
{
    ...
    public function view()
    {
        ...
        if (request('route') == 'draft') {
            return view('mail.compose', compact('email'));
        } else {
            return view('mail.view', compact('email'));
        }
    }
    ...
    public function store()
    {
        ...
        if (request('is_draft')) {
            ...
            if (request()->ajax()) {
                return [
                    'success' => trans('app.mail.saved-to-draft')
                ];
            } else {
        ...
        if (request()->ajax()) {
            return [
                'success' => trans('app.mail.create-success')
            ];
        } else {
            return redirect()->route('admin.mail.index', ['route'   => 'inbox']);
        }
        ...
        if (request()->ajax()) {
            ...
            if (request('lead_id')) {
                $response['html'] = view('common.custom-attributes.view', [
                    'customAttributes' => app('App\Attribute\Repositories\AttributeRepository')->findWhere([
                        'entity_type' => 'leads',
                    ]),
