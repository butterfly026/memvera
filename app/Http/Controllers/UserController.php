...
class UserController extends Controller
{
     ...
     *
     * @var \App\Repositories\User\RoleRepository
     */
     ...
     * @param  \App\Repositories\User\GroupRepository  $groupRepository
     * @param  \App\Repositories\User\RoleRepository  $roleRepository
     * @return void
     */
    ...
    public function edit($id)
    {
        ...
        return view('settings.users.edit', compact('admin', 'groups', 'roles'));
    }
    ...
    public function destroy($id)
    {
        if (auth()->user()->id == $id) {
            return response()->json([
                'message' => trans('app.settings.users.delete-failed'),
            ], 400);
        } else if ($this->userRepository->count() == 1) {
            return response()->json([
                'message' => trans('app.settings.users.last-delete-error'),
            ], 400);
        } else {
            ...
            try {
                ...
                return response()->json([
                    'message' => trans('app.settings.users.delete-success'),
                ]);
    ...
    public function massUpdate()
    {
        ...
        if (! $count) {
            return response()->json([
                'message' => trans('app.settings.users.mass-update-failed'),
            ], 400);
        }

        return response()->json([
            'message' => trans('app.settings.users.mass-update-success'),
        ]);
    }
    ...
    public function massDestroy()
    {
        ...
        if (! $count) {
            return response()->json([
                'message' => trans('app.settings.users.mass-delete-failed'),
            ], 400);
        }

        return response()->json([
            'message' => trans('app.settings.users.mass-delete-success'),
        ]);
    }
}
